#!/usr/bin/env bash
# ============================================================================
#  Maqueen Lite Field Guide Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/app-maqueen-activities}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.maqueenactivities}"
AVD_NAME="${AVD_NAME:-maqueen_activities_test}"
...
cmd_test() {
  ...
  local out="/tmp/maqueen-activities-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Maqueen Lite Field Guide Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
